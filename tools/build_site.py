...
<script>
  ...
  function goAyah(){
    ...
    if(!card) return;
    card.open=true;
    // Programmatic open does NOT fire the 'toggle' event, so fill the surah
    // texts explicitly (offline, so this is instant — no network needed).
    [].slice.call(card.querySelectorAll('.a-txt[data-load]')).forEach(function(box){
      var ayahEl=box.closest('.ayah');
      fillText(num,+ayahEl.dataset.ayah,ayahEl);
    });
    function focusAyah(tries){
      ...
      // ensure this ayah's text is filled (in case it was added late)
