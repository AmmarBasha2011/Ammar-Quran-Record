...
<script>
  ...
  // NOTE: must never throw — a thrown error here would stop the whole script
  // (including the deep-link handler). Guard fetch in try/catch.
  function fillText(surah,ayah,el){
    // normalise surah key: TEXTS is keyed by zero-padded string e.g. "024"
    var key=('000'+surah).slice(-3);
    var box=el.querySelector('.a-txt');
    var t=(TEXTS[key] && TEXTS[key][ayah-1]!=null) ? TEXTS[key][ayah-1] : '';
    if(t){ box.textContent=t; box.removeAttribute('data-load'); return; }
    // text not in offline bundle — try API, but never throw
    if(typeof fetch==='undefined'){ box.textContent='—'; box.removeAttribute('data-load'); return; }
    ...
    fetch('https://api.quran.com/api/v4/quran/verses/uthmani?chapter_number='+surah)
      ...
      .then(function(d){
        if(d.verses) d.verses.forEach(function(v){
          ...
          var pk=('000'+p[0]).slice(-3);
          if(!TEXTS[pk]) TEXTS[pk]=[];
          TEXTS[pk][p[1]-1]=v.text_uthmani;
        });
        box.textContent=(TEXTS[key]&&TEXTS[key][ayah-1])?TEXTS[key][ayah-1]:'—';
  ...
  // ---- fill ALL embedded texts immediately on load (offline, instant) ----
  function fillAllTexts(){
    [].slice.call(list.querySelectorAll('.a-txt[data-load]')).forEach(function(box){
      var ayahEl=box.closest('.ayah');
      var surah=+ayahEl.closest('.surah').dataset.num;
      fillText(surah,+ayahEl.dataset.ayah,ayahEl);
    });
  }
